...
def send_cashier_logout_notification(cashier_id):
    ...
    try:
        current_app.logger.info(f"Starting cashier logout notification for ID: {cashier_id}")
        ...
        current_app.logger.info(f"Cashier logout notifications sent: {emails_sent} emails, WebSocket notifications sent")
        return emails_sent > 0
        
    except Exception as e:
        current_app.logger.error(f"Error sending cashier logout notification: {str(e)}")
        return False
...
def send_test_notification(admin_email):
    ...
    try:
        ...
        error_msg = str(e)
        current_app.logger.error(f"Failed to send test notification: {error_msg}")
        
        # Provide helpful error messages
        if "authentication" in error_msg.lower() or "auth" in error_msg.lower():
            return False, "Authentication failed. For Gmail, use App Password instead of regular password."
        elif "connection" in error_msg.lower() or "connect" in error_msg.lower():
            return False, "Connection failed. Check your mail server and port settings."
        elif "tls" in error_msg.lower() or "ssl" in error_msg.lower():
            return False, "TLS/SSL error. Check your encryption settings."
